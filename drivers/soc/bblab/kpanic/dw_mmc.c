		...
		u32 desc0, u32 desc1, u32 desc2)
{
	...
	desc->flags = desc0;
	desc->cnt = desc1;
	desc->addr = virt_to_phys((void *)desc2);
	desc->next_addr = virt_to_phys((void *)desc + sizeof(struct dwmci_idmac));
}
			       ...
			       void *bounce_buffer)
{
	...
	dwmci_writel(host, DWMCI_DBADDR, virt_to_phys((void *)cur_idmac));
	...
	//__cpuc_flush_dcache_area((void *)data_start, data_end - data_start + ARCH_DMA_MINALIGN);
	dmac_flush_range((void *)data_start, (void *)(data_end + ARCH_DMA_MINALIGN));
...
static int dwmci_get_cd(struct udevice *dev)
{
	struct mmc *mmc = mmc_get_mmc_dev(dev);
#else
static int dwmci_get_cd(struct mmc *mmc)
{
#endif
	int ret = -1;
...
int add_dwmci(struct dwmci_host *host, u32 max_clk, u32 min_clk)
{
	...
	host->mmc = mmc_create(&host->cfg, host->mmc); // tricky, use host->mmc as priv
	if (host->mmc == NULL)
		return -1;
