...
static bool mmc_can_card_busy(struct mmc *mmc)
{
	return !!mmc->cfg->ops->card_busy(mmc);
}
#endif

int emmc_send_status(struct mmc *mmc, int timeout)
{
	struct mmc_cmd cmd;
	int err, retries = 5;
...
int emmc_set_blocklen(struct mmc *mmc, int len)
{
	struct mmc_cmd cmd;
...
#endif
{
	...
	if (emmc_set_blocklen(mmc, mmc->read_bl_len)) {
...
re_init_retry:
		...
		if (mmc_read_blocks_prepare(mmc, dst, start, blkcnt) != blkcnt) {
			printk("%s: Re-init mmc_read_blocks_prepare error\n",
			       __func__);
			goto re_init_retry;
		}
	}
...
re_init_retry:
			...
			if (mmc_read_blocks(mmc, dst, start, cur) != cur) {
				printk("%s: Re-init mmc_read_blocks error\n",
				       __func__);
				goto re_init_retry;
			}
		}
...
//static u8 tuning_blk_pattern[128] __aligned(ARCH_DMA_MINALIGN);
static u8 tuning_blk_pattern[512] __aligned(512);
int emmc_send_tuning(struct mmc *mmc, u32 opcode)
{
	struct mmc_cmd cmd;
	struct mmc_data data;
	...
	data_buf = tuning_blk_pattern;
	if (!data_buf)
		return -ENOMEM;

    memset(data_buf, 0, sizeof(tuning_blk_pattern));
		      ...
		      enum mmc_hwpart_conf_mode mode)
{
	...
	if (IS_SD(mmc) || (mmc->version < MMC_VERSION_4_41)) {
		printk("eMMC >= 4.4 required for enhanced user data area\n");
		return -EMEDIUMTYPE;
	}

	if (!(mmc->part_support & PART_SUPPORT)) {
		printk("Card does not support partitioning\n");
		return -EMEDIUMTYPE;
	}

	if (!mmc->hc_wp_grp_size) {
		printk("Card does not define HC WP group size\n");
		return -EMEDIUMTYPE;
	}
	...
	if (conf->user.enh_size) {
		if (conf->user.enh_size % mmc->hc_wp_grp_size ||
			...
			printk("User data enhanced area not HC WP group "
			       "size aligned\n");
			return -EINVAL;
		}
	...
	for (pidx = 0; pidx < 4; pidx++) {
		if (conf->gp_part[pidx].size % mmc->hc_wp_grp_size) {
			printk("GP%i partition not HC WP group size "
			       "aligned\n", pidx+1);
			return -EINVAL;
		}
	...
	if (part_attrs && ! (mmc->part_support & ENHNCD_SUPPORT)) {
		printk("Card does not support enhanced attribute\n");
		return -EMEDIUMTYPE;
	}
	...
	if (tot_enh_size_mult > max_enh_size_mult) {
		printk("Total enhanced size exceeds maximum (%u > %u)\n",
	...
	if (wr_rel_set != ext_csd[EXT_CSD_WR_REL_SET] &&
		...
		printk("Card does not support host controlled partition write "
	...
	if (ext_csd[EXT_CSD_PARTITION_SETTING] &
		...
		printk("Card already partitioned\n");
		return -EPERM;
	}
...
static int mmc_startup(struct mmc *mmc)
{
    ...
    debug("%s: cid = 0x%x, 0x%x, 0x%x, 0x%x\n", __func__, mmc->cid[0], mmc->cid[1], mmc->cid[2], mmc->cid[3]);

	/*
	 * For MMC cards, set the Relative Address.
	 * For SD cards, get the Relatvie Address.
	 * This also printk the cards into Standby State
	 */
	...
	bdesc->lba = lldiv(mmc->capacity, mmc->read_bl_len);
		...
		!defined(CONFIG_USE_TINY_printk))
...
static int mmc_power_init(struct mmc *mmc)
{
	...
	if (ret) {
		printk("Error enabling VMMC supply\n");
		return ret;
	}
#endif
#else /* !CONFIG_DM_MMC */
...
#if 0
static int mmc_set_dsr(struct mmc *mmc, u16 val)
{
	mmc->dsr = val;
	return 0;
}
...
__weak int cpu_mmc_init()
{
	return -1;
}

/* board-specific MMC initializations. */
__weak int board_mmc_init()
{
	return -1;
}
...
int mmc_set_bkops_enable(struct mmc *mmc)
{
	...
	err = mmc_send_ext_csd(mmc, ext_csd);
	if (err) {
		printk("Could not get ext_csd register values\n");
		return err;
	}

	if (!(ext_csd[EXT_CSD_BKOPS_SUPPORT] & 0x1)) {
		printk("Background operations not supported on device\n");
		return -EMEDIUMTYPE;
	}

	if (ext_csd[EXT_CSD_BKOPS_EN] & 0x1) {
		printk("Background operations already enabled\n");
		return 0;
	}

	err = mmc_switch(mmc, EXT_CSD_CMD_SET_NORMAL, EXT_CSD_BKOPS_EN, 1);
	if (err) {
		printk("Failed to enable manual background operations\n");
		return err;
	}

	printk("Enabled manual background operations\n");
