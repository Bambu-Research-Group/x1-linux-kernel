...
dhd_alive_postinit(dhd_pub_t *dhd)
{
	...
	memset(buf, 0, sizeof(buf));
	ptr = buf;
	dhd_conf_get_iovar(dhd, 0, WLC_GET_VAR, "ver", (char *)buf, sizeof(buf));
	if (!ret) {
		bcmstrtok(&ptr, "\n", 0);
...
#else
	DHD_PRINT("%s: SensorHub diabled %d\n",
			__FUNCTION__, dhd->info->shub_enable);
	dhd->info->shub_enable = FALSE;
	shub_ctl.enable = FALSE;
	ret2 = dhd_iovar(dhd, 0, "shub", (char *)&shub_ctl, sizeof(shub_ctl),
			NULL, 0, TRUE);
	if (ret2 < 0) {
		DHD_PRINT("%s failed to set ShubHub disable\n",
			__FUNCTION__);
	}
#endif /* SUPPORT_SENSORHUB */
...
void dhd_get_memdump_info(dhd_pub_t *dhd)
{
	...
	if (IS_ERR(fp)) {
		DHD_PRINT("%s: File [%s] doesn't exist\n", __FUNCTION__, filepath);
#if defined(CONFIG_X86)
